//
//  GenearlLedgerAccountModel.swift
//  Accountant
//
...
// 勘定クラス
class GenearlLedgerAccountModel: GenearlLedgerAccountModelInput {
...
//                    print("勘定が存在しない")
    ...
    func getJournalEntryInAccount(account: String) -> Results<DataBaseJournalEntry> {
        ...
        let dataBaseAccountingBook = realm.objects(DataBaseAccountingBooks.self).filter("openOrClose == \(true)").first
        if account == "損益勘定" {
            // 損益勘定の場合
