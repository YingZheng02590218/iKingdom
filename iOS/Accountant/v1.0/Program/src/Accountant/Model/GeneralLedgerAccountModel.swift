...
class GeneralLedgerAccountModel: GeneralLedgerAccountModelInput {
    ...
    // 初期化 PDFメーカー
    func initializePdfMaker(yearMonth: String? = nil, account: String, completion: (URL?) -> Void) {
        
        pDFMaker.initialize(yearMonth: yearMonth, account: account, completion: { filePath in
            completion(filePath)
        })
    }
