    ...
    func readMasterDataFromCSVOfBSAndPL() {
        if let csvPath = Bundle.main.path(forResource: "MasterDataBSAndPL", ofType: "csv") {
            ...
            csvString.enumerateLines { (line, stop) -> () in
                ...
                dataBaseSettingsCategoryBSAndPL.BSAndPL_category = Int(line.components(separatedBy:",")[3])! //表記名
                dataBaseSettingsCategoryBSAndPL.category = line.components(separatedBy:",")[4]//表記名
