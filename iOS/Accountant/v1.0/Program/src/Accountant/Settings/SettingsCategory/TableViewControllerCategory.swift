...
//        cell.label_category.text = " \(objects[indexPath.row].category as String)"
    ...
    // データベース
        ...
        // モデルオブフェクトの取得 勘定別に取得
        let objectss = dataBaseManagerAccount.getAllAccount(account: objects[indexPath.row].category as String)//通常仕訳
        let objectsss = dataBaseManagerAccount.getAllAccountAdjusting(account: objects[indexPath.row].category as String)//決算整理仕訳
